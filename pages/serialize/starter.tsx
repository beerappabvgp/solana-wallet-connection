...
const Starter: FC = () => {
  ...
  const [studentIntros, setStudentIntros] = React.useState<
    StudentIntro[]
  >([]);
  ...
  const handleTransactionSubmit = async (
    ...
    try {
      ...
      console.log(
        `Transaction submitted: https://explorer.solana.com/tx/${response}?cluster=devnet`
      );
      ...
      <section className="mb-4 grid grid-cols-1 sm:grid-cols-6 gap-4 px-4">
        <div className="rounded-lg min-h-content p-4 bg-[#2a302f] sm:col-span-6 lg:col-start-2 lg:col-end-6">
          ...
          <div>
            <div className="mt-6">
              {studentIntros.map(
                (studentIntro: StudentIntro, index: number) =>
                  studentIntro.name &&
                  studentIntro.message && (
                    ...
                    >
                      ...
                      <p className="text-sm mt-2">{studentIntro.message}</p>
                    </div>
                  )
              )}
            </div>
            <div className="mt-6 flex justify-between">
              ...
              <div>
                {StudentIntroCoordinator.accounts.length >
                  page * 5 && (
                  <button
                    onClick={() => setPage(page + 1)}
                    className="bg-helius-orange rounded-lg w-24 py-1 font-semibold transition-all duration-200 hover:bg-transparent border-2 border-transparent hover:border-helius-orange"
